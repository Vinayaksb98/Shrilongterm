...
if "matches" in st.session_state:
    ...
    if matches.empty:
        ...
        st.dataframe(display[action_cols].round(2), use_container_width=True, hide_index=True)
        buys=display[display["Buy Call"].astype(str).str.startswith("BUY")].copy()
        if not buys.empty:
            st.markdown("### 🟢 Actionable BUY candidates")
            st.dataframe(buys[["Symbol","Timeframe","Buy Call","Confidence","Current","Stop Loss","Target 1","Target 2","Support","Resistance","Volume Ratio","R:R T1"]].round(2), use_container_width=True, hide_index=True)
        else:
