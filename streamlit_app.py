import streamlit as st
import pandas as pd
import numpy as np
import yfinance as yf
import requests
import io
from concurrent.futures import ThreadPoolExecutor, as_completed
from datetime import datetime

st.set_page_config(page_title="NIFTY 500 Momentum Scanner — Trade Plan", layout="wide")
APP_VERSION = "trade-plan-v3"

OFFICIAL_NIFTY500_URL = "https://www.nseindia.com/content/indices/ind_nifty500list.csv"
FALLBACK_NIFTY500_URL = "https://raw.githubusercontent.com/ganeshbiyer/Nse_Historical_Data/main/nifty500_symbols.csv"

TIMEFRAME_MAP = {
    "15 Min": {"interval": "15m", "period": "60d"},
    "1 Hour": {"interval": "1h", "period": "730d"},
    "1 Day": {"interval": "1d", "period": "2y"},
}

HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120 Safari/537.36",
    "Accept": "text/csv,text/plain,*/*",
}

@st.cache_data(ttl=24*60*60, show_spinner=False)
def get_nifty500_universe():
    errors = []
    try:
        s = requests.Session()
        s.headers.update(HEADERS)
        s.get("https://www.nseindia.com", timeout=15)
        r = s.get(OFFICIAL_NIFTY500_URL, timeout=30)
        r.raise_for_status()
        df = pd.read_csv(io.StringIO(r.text))
        if "Symbol" in df.columns:
            symbols = df["Symbol"].dropna().astype(str).str.strip().tolist()
            return pd.DataFrame({"symbol": symbols, "source": "NSE official"})
    except Exception as e:
        errors.append(str(e))
    try:
        r = requests.get(FALLBACK_NIFTY500_URL, timeout=30)
        r.raise_for_status()
        df = pd.read_csv(io.StringIO(r.text))
        col = "Symbol" if "Symbol" in df.columns else df.columns[0]
        symbols = df[col].dropna().astype(str).str.strip().tolist()
        return pd.DataFrame({"symbol": symbols, "source": "fallback mirror"})
    except Exception as e:
        errors.append(str(e))
        raise RuntimeError("Could not download NIFTY 500 list. " + " | ".join(errors))

@st.cache_data(ttl=6*60*60, show_spinner=False)
def get_market_cap(symbol):
    ticker = yf.Ticker(symbol + ".NS")
    try:
        fi = ticker.fast_info
        cap = getattr(fi, "market_cap", None)
        if cap is None and isinstance(fi, dict):
            cap = fi.get("market_cap")
        if cap is not None and np.isfinite(float(cap)):
            return float(cap)
    except Exception:
        pass
    try:
        info = ticker.info
        cap = info.get("marketCap")
        return float(cap) if cap else np.nan
    except Exception:
        return np.nan

@st.cache_data(ttl=6*60*60, show_spinner=False)
def get_large_cap_universe(symbols, min_market_cap):
    out = []
    with ThreadPoolExecutor(max_workers=12) as ex:
        futures = {ex.submit(get_market_cap, s): s for s in symbols}
        for fut in as_completed(futures):
            s = futures[fut]
            try:
                cap = fut.result()
                if pd.notna(cap) and cap >= min_market_cap:
                    out.append((s, cap))
            except Exception:
                pass
    df = pd.DataFrame(out, columns=["symbol", "market_cap"])
    if df.empty:
        return pd.DataFrame(columns=["symbol", "market_cap"])
    return df.sort_values("market_cap", ascending=False).reset_index(drop=True)

def ema(s, span):
    return s.ewm(span=span, adjust=False).mean()

def rsi(close, period=14):
    delta = close.diff()
    gain = delta.clip(lower=0).ewm(alpha=1/period, adjust=False).mean()
    loss = (-delta.clip(upper=0)).ewm(alpha=1/period, adjust=False).mean()
    rs = gain / loss.replace(0, np.nan)
    return 100 - (100 / (1 + rs))

def stochastic(high, low, close, k_period=14, k_smooth=3, d_period=3):
    lowest = low.rolling(k_period).min()
    highest = high.rolling(k_period).max()
    raw_k = 100 * (close - lowest) / (highest - lowest).replace(0, np.nan)
    k = raw_k.rolling(k_smooth).mean()
    d = k.rolling(d_period).mean()
    return k, d

def atr(df, period=14):
    high = df["High"].astype(float)
    low = df["Low"].astype(float)
    close = df["Close"].astype(float)
    prev = close.shift(1)
    tr = pd.concat([high-low, (high-prev).abs(), (low-prev).abs()], axis=1).max(axis=1)
    return tr.rolling(period).mean()

def support_resistance(df, lookback=120, pivot_window=3):
    d = df.tail(lookback).copy()
    h = d["High"].astype(float)
    l = d["Low"].astype(float)
    c = d["Close"].astype(float)
    price = float(c.iloc[-1])

    pl = l.eq(l.rolling(2*pivot_window+1, center=True).min())
    ph = h.eq(h.rolling(2*pivot_window+1, center=True).max())
    lows = l[pl].dropna().tolist()
    highs = h[ph].dropna().tolist()

    def cluster(vals, tol=.012):
        vals = sorted(float(v) for v in vals if np.isfinite(v))
        out = []
        for v in vals:
            if not out or abs(v-np.mean(out[-1]))/max(np.mean(out[-1]), 1e-9) > tol:
                out.append([v])
            else:
                out[-1].append(v)
        return [float(np.mean(x)) for x in out]

    ss = [v for v in cluster(lows) if v < price*.995]
    rr = [v for v in cluster(highs) if v > price*1.005]

    e20 = float(ema(c, 20).iloc[-1])
    if e20 < price and all(abs(e20-s)/price > .01 for s in ss):
        ss.append(e20)

    ss = sorted(ss, reverse=True)
    rr = sorted(rr)
    support = ss[0] if ss else float(l.tail(30).min())
    resistance = rr[0] if rr else float(h.tail(30).max())
    strong = rr[1] if len(rr) > 1 else resistance
    return support, resistance, strong

def trade_plan(ind):
    x = ind.iloc[-1]
    close = float(x["Close"])
    vol = float(x["Volume"])
    vs = float(x["VOL_SMA20"]) if pd.notna(x["VOL_SMA20"]) else 0
    a = float(x["ATR14"]) if pd.notna(x["ATR14"]) else max(close*.02, .01)

    support, resistance, strong = support_resistance(ind)
    vr = vol/vs if vs > 0 else 0

    zone_high = min(close, max(support, close - 0.50*a))
    zone_low = max(support - 0.50*a, close - 1.25*a)
    if zone_low >= zone_high:
        zone_low = max(close - 1.0*a, close*.96)
        zone_high = close

    trigger_base = resistance if resistance > close else close + 0.50*a
    trigger = trigger_base * 1.002

    risk_sl = close - 1.5*a
    stop = max(min(support - .35*a, risk_sl), close*.94)

    risk = max(close-stop, .01)
    t1 = resistance if resistance > close else close + 1.5*risk
    if t1 <= close+risk:
        t1 = close + 1.5*risk
    t2 = strong if strong > t1 else max(t1+risk, close+2.5*risk)

    breakout = close > resistance*1.002
    near_support = abs(close-support)/close <= .025

    if breakout and vr >= 1.15:
        action, conf = "BUY — BREAKOUT", "High"
    elif near_support and vr >= 1.15:
        action, conf = "BUY", "High"
    elif near_support:
        action, conf = "BUY ON VOLUME CONFIRMATION", "Medium"
    else:
        action, conf = "WAIT", "Low/Medium"

    return {
        "Buy Call": action,
        "Confidence": conf,
        "Buy Zone": f"{zone_low:.2f} – {zone_high:.2f}",
        "Buy Zone Low": zone_low,
        "Buy Zone High": zone_high,
        "Trigger": trigger,
        "Stop Loss": stop,
        "Target 1": t1,
        "Target 2": t2,
        "Support": support,
        "Resistance": resistance,
        "Strong Resistance": strong,
        "Volume Ratio": vr,
        "R:R T1": (t1-close)/risk,
        "R:R T2": (t2-close)/risk,
        "Risk %": risk/close*100,
        "Upside T1 %": (t1/close-1)*100,
        "Upside T2 %": (t2/close-1)*100,
    }

def indicators(df):
    df = df.copy().dropna()
    if len(df) < 60:
        return None

    close = df["Close"].astype(float)
    high = df["High"].astype(float)
    low = df["Low"].astype(float)
    vol = df["Volume"].astype(float)

    df["EMA20"] = ema(close, 20)
    df["EMA50"] = ema(close, 50)

    macd_line = ema(close, 12) - ema(close, 26)
    signal = ema(macd_line, 9)
    df["MACD"] = macd_line
    df["MACD_HIST"] = macd_line - signal

    df["RSI14"] = rsi(close, 14)
    df["STOCH_K"], df["STOCH_D"] = stochastic(high, low, close, 14, 3, 3)
    df["VOL_SMA20"] = vol.rolling(20).mean()
    df["ATR14"] = atr(df, 14)
    return df

def evaluate(df):
    x = df.iloc[-1]
    checks = {
        "Close > EMA20": x["Close"] > x["EMA20"],
        "EMA20 > EMA50": x["EMA20"] > x["EMA50"],
        "MACD > 0": x["MACD"] > 0,
        "RSI 55-70": 55 < x["RSI14"] < 70,
        "Stochastic 40-75": 40 < x["STOCH_K"] < 75,
        "Volume > SMA20": x["Volume"] > x["VOL_SMA20"],
    }
    score = sum(bool(v) for v in checks.values())
    return checks, score, x

@st.cache_data(ttl=15*60, show_spinner=False)
def analyze_symbol(symbol, timeframe):
    cfg = TIMEFRAME_MAP[timeframe]
    try:
        df = yf.download(
            symbol + ".NS",
            period=cfg["period"],
            interval=cfg["interval"],
            progress=False,
            auto_adjust=False,
            threads=False,
        )
        if df is None or df.empty:
            return None
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = [c[0] for c in df.columns]

        need = ["Open", "High", "Low", "Close", "Volume"]
        if not all(c in df.columns for c in need):
            return None

        ind = indicators(df[need])
        if ind is None:
            return None

        checks, score, x = evaluate(ind)
        plan = trade_plan(ind)

        return {
            "symbol": symbol,
            "timeframe": timeframe,
            "match": all(checks.values()),
            "score": score,
            "close": float(x["Close"]),
            "EMA20": float(x["EMA20"]),
            "EMA50": float(x["EMA50"]),
            "MACD": float(x["MACD"]),
            "MACD_HIST": float(x["MACD_HIST"]),
            "RSI14": float(x["RSI14"]),
            "STOCH_K": float(x["STOCH_K"]),
            "STOCH_D": float(x["STOCH_D"]),
            "volume": float(x["Volume"]),
            "vol_sma20": float(x["VOL_SMA20"]) if pd.notna(x["VOL_SMA20"]) else np.nan,
            "ATR14": float(x["ATR14"]) if pd.notna(x["ATR14"]) else np.nan,
            **plan,
            "checks": checks,
            "as_of": str(ind.index[-1]),
        }
    except Exception:
        return None

def scan(universe, timeframes, match_mode, progress_cb):
    rows = []
    tasks = [(s, tf) for s in universe["symbol"].tolist() for tf in timeframes]
    total = len(tasks)
    if total == 0:
        return pd.DataFrame(), pd.DataFrame()

    done = 0
    with ThreadPoolExecutor(max_workers=8) as ex:
        futures = {ex.submit(analyze_symbol, s, tf): (s, tf) for s, tf in tasks}
        for fut in as_completed(futures):
            done += 1
            progress_cb(done / total, f"Analyzed {done}/{total} symbol-timeframe combinations")
            try:
                result = fut.result()
                if result:
                    rows.append(result)
            except Exception:
                pass

    if not rows:
        return pd.DataFrame(), pd.DataFrame()

    all_df = pd.DataFrame(rows)
    cap_map = universe.set_index("symbol")["market_cap"].to_dict()
    all_df["market_cap"] = all_df["symbol"].map(cap_map)

    if match_mode == "ALL selected timeframes":
        pivot = all_df.pivot_table(index="symbol", columns="timeframe", values="match", aggfunc="first").fillna(False)
        good_symbols = pivot.index[pivot.reindex(columns=timeframes, fill_value=False).all(axis=1)].tolist()
    else:
        good_symbols = all_df.loc[all_df["match"], "symbol"].unique().tolist()

    matches = all_df[all_df["symbol"].isin(good_symbols)].copy()
    matches = matches.sort_values(["score", "market_cap"], ascending=[False, False])
    return matches, all_df

st.title("📈 NIFTY 500 Bullish Momentum Scanner")
st.caption("Yahoo Finance data • NIFTY 500 universe • Market cap filter • 15 Min / 1 Hour / 1 Day")

with st.sidebar:
    st.header("Scanner Settings")
    selected_tfs = st.multiselect(
        "Timeframe filters",
        ["15 Min", "1 Hour", "1 Day"],
        default=["1 Day"],
        help="The strategy rules are evaluated independently on every selected timeframe."
    )
    match_mode = st.radio("Multi-timeframe rule", ["ANY selected timeframe", "ALL selected timeframes"])
    min_cap_cr = st.number_input(
        "Minimum market cap (₹ Crore)",
        min_value=1000,
        value=10000,
        step=1000
    )
    st.divider()
    st.markdown("### Strategy rules")
    st.markdown("""
    1. Close > EMA 20
    2. EMA 20 > EMA 50
    3. MACD Line > 0
    4. RSI 14 > 55 and < 70
    5. Stochastic (14,3,3) > 40 and < 75
    6. Volume > SMA(Volume,20)
    """)
    refresh = st.button("🔄 Refresh universe & data", use_container_width=True)

if refresh:
    st.cache_data.clear()
    st.session_state.pop("eligible_universe", None)

try:
    universe_raw = get_nifty500_universe()
except Exception as e:
    st.error(f"Unable to load NIFTY 500 universe: {e}")
    st.stop()

source = universe_raw["source"].iloc[0]
st.info(f"Universe source: {source}. NIFTY 500 list refreshes automatically every 24 hours.")

col1, col2, col3 = st.columns(3)
col1.metric("NIFTY universe", len(universe_raw))
col2.metric("Market-cap threshold", f"₹{min_cap_cr:,.0f} Cr")
col3.metric("Selected timeframes", len(selected_tfs))

if not selected_tfs:
    st.warning("Select at least one timeframe.")
    st.stop()

st.markdown("### Step 1 — Build eligible universe")
if "eligible_universe" not in st.session_state or refresh:
    with st.spinner("Checking market capitalisation from Yahoo Finance..."):
        st.session_state["eligible_universe"] = get_large_cap_universe(
            tuple(universe_raw["symbol"].tolist()),
            min_cap_cr * 10_000_000
        )

eligible = st.session_state["eligible_universe"]
st.success(f"Eligible stocks with market cap ≥ ₹{min_cap_cr:,.0f} Cr: {len(eligible)}")

with st.expander("View eligible universe"):
    show_u = eligible.copy()
    show_u["market_cap_cr"] = show_u["market_cap"] / 10_000_000
    st.dataframe(show_u[["symbol", "market_cap_cr"]], use_container_width=True, hide_index=True)

run = st.button("▶ Run Momentum Scan", type="primary", use_container_width=True)

if run:
    progress = st.progress(0, text="Starting scan...")
    def update(p, txt):
        progress.progress(min(p, 1.0), text=txt)

    matches, all_results = scan(eligible, selected_tfs, match_mode, update)
    progress.empty()
    st.session_state["matches"] = matches
    st.session_state["all_results"] = all_results
    st.session_state["scan_time"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

if "matches" in st.session_state:
    matches = st.session_state["matches"]
    all_results = st.session_state["all_results"]

    st.markdown(f"## Scan Results — {st.session_state['scan_time']}")
    st.caption("Score, Volume Ratio and R:R columns are hidden from the trading tables.")

    if matches.empty:
        st.warning("No stocks matched all required conditions.")
    else:
        display = matches.copy()
        display["Market Cap (₹ Cr)"] = display["market_cap"] / 10_000_000
        display = display.rename(columns={
            "symbol": "Symbol",
            "timeframe": "Timeframe",
            "close": "Current",
        })

        action_cols = [
            "Symbol", "Timeframe", "Buy Call", "Confidence",
            "Current", "Buy Zone", "Trigger", "Stop Loss",
            "Target 1", "Target 2", "Support", "Resistance",
            "Strong Resistance"
        ]

        st.markdown("### 🎯 Buy Call + Buy Zone + Trigger + Stop Loss + Targets")
        st.caption("Buy Zone is a price range. Trigger is the breakout/confirmation level.")

        for col in action_cols:
            if col not in display.columns:
                display[col] = np.nan

        action_view = display[action_cols].copy()
        numeric_cols = [
            "Current", "Trigger", "Stop Loss", "Target 1", "Target 2",
            "Support", "Resistance", "Strong Resistance"
        ]
        for col in numeric_cols:
            action_view[col] = pd.to_numeric(action_view[col], errors="coerce").round(2)

        st.dataframe(action_view, use_container_width=True, hide_index=True)

        buys = action_view[
            action_view["Buy Call"].fillna("").astype(str).str.startswith("BUY")
        ].copy()

        if not buys.empty:
            st.markdown("### 🟢 Actionable BUY candidates")
            buy_cols = [
                "Symbol", "Timeframe", "Buy Call", "Confidence",
                "Current", "Buy Zone", "Trigger", "Stop Loss",
                "Target 1", "Target 2", "Support", "Resistance"
            ]
            st.dataframe(buys[buy_cols], use_container_width=True, hide_index=True)
        else:
            st.info("No candidate currently has enough support/volume/reward confirmation for a BUY call.")

        csv = display.to_csv(index=False).encode("utf-8")
        st.download_button(
            "⬇ Download results CSV",
            csv,
            "nifty500_momentum_results.csv",
            "text/csv"
        )

    with st.expander("All analyzed results (including non-matches)"):
        if not all_results.empty:
            st.dataframe(
                all_results.drop(columns=["checks"], errors="ignore"),
                use_container_width=True,
                hide_index=True
            )

st.divider()
st.caption(
    "Data note: Yahoo Finance intraday data availability and retention limits can vary. "
    "The app refreshes market data on reruns/cache expiry. Streamlit Community Cloud local storage is not a permanent database."
)
