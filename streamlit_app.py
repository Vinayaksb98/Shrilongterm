...
st.set_page_config(page_title="NIFTY 500 Momentum Scanner — Trade Plan", layout="wide")
APP_VERSION = "trade-plan-v2"
...
def support_resistance(df, lookback=120, pivot_window=3):
    d=df.tail(lookback).copy()
    h,l,c=d["High"].astype(float),d["Low"].astype(float),d["Close"].astype(float)
    price=float(c.iloc[-1])
    pl=l.eq(l.rolling(2*pivot_window+1, center=True).min())
    ph=h.eq(h.rolling(2*pivot_window+1, center=True).max())
    lows=l[pl].dropna().tolist(); highs=h[ph].dropna().tolist()
    def cluster(vals, tol=.012):
        vals=sorted(float(v) for v in vals if np.isfinite(v)); out=[]
        for v in vals:
            if not out or abs(v-np.mean(out[-1]))/max(np.mean(out[-1]),1e-9)>tol: out.append([v])
            else: out[-1].append(v)
        return [float(np.mean(x)) for x in out]
    ss=[v for v in cluster(lows) if v<price*.995]
    rr=[v for v in cluster(highs) if v>price*1.005]
    e20=float(ema(c,20).iloc[-1])
    if e20<price and all(abs(e20-s)/price>.01 for s in ss): ss.append(e20)
    ss=sorted(ss,reverse=True); rr=sorted(rr)
    support=ss[0] if ss else float(l.tail(30).min())
    resistance=rr[0] if rr else float(h.tail(30).max())
    strong=rr[1] if len(rr)>1 else resistance
    return support,resistance,strong

def trade_plan(ind):
    x=ind.iloc[-1]; close=float(x["Close"]); vol=float(x["Volume"]); vs=float(x["VOL_SMA20"])
    a=float(x["ATR14"]) if pd.notna(x["ATR14"]) else max(close*.02,.01)
    support,resistance,strong=support_resistance(ind)
    vr=vol/vs if vs>0 else 0
    breakout=close>resistance*1.002
    near_support=abs(close-support)/close<=.025
    risk_sl=close-1.5*a
    stop=max(min(support-.35*a,risk_sl),close*.94)
    risk=max(close-stop,.01)
    t1=resistance if resistance>close else close+1.5*risk
    if t1<=close+risk: t1=close+1.5*risk
    t2=strong if strong>t1 else max(t1+risk,close+2.5*risk)
    rr1=(t1-close)/risk; rr2=(t2-close)/risk
    if breakout and vr>=1.15 and rr1>=1.5: action,conf="BUY — BREAKOUT","High"
    elif (near_support or close>float(x["EMA20"])) and vr>=1.15 and rr1>=1.5: action,conf="BUY","High"
    elif (near_support or close>float(x["EMA20"])) and rr1>=1.5: action,conf="BUY ON VOLUME CONFIRMATION","Medium"
    else: action,conf="WAIT","Low/Medium"
    return {"Buy Call":action,"Confidence":conf,"Buy Zone":close,"Stop Loss":stop,
            "Target 1":t1,"Target 2":t2,"Support":support,"Resistance":resistance,
            "Strong Resistance":strong,"Volume Ratio":vr,"R:R T1":rr1,"R:R T2":rr2,
            "Risk %":risk/close*100,"Upside T1 %":(t1/close-1)*100,"Upside T2 %":(t2/close-1)*100}
...
if "matches" in st.session_state:
    ...
    if matches.empty:
        ...
        action_cols = ["Symbol", "Timeframe", "Score", "Buy Call", "Confidence",
                       "Current", "Buy Zone", "Stop Loss", "Target 1", "Target 2",
                       "Support", "Resistance", "Strong Resistance", "Volume Ratio",
                       "R:R T1", "R:R T2", "Risk %", "Upside T1 %", "Upside T2 %"]
        st.markdown("### 🎯 Buy Call + Stop Loss + Targets")
        st.caption("Levels use recent pivot support/resistance, EMA20 dynamic support, ATR14 risk spacing and relative-volume confirmation.")

        # Defensive handling for cached/older scan rows: never crash if a new
        # trade-plan field is absent. Missing fields are shown as blank.
        for col in action_cols:
            if col not in display.columns:
                display[col] = np.nan
        action_view = display[action_cols].copy()
        numeric_cols = [c for c in action_cols if c not in ["Symbol", "Timeframe", "Buy Call", "Confidence"]]
        for col in numeric_cols:
            action_view[col] = pd.to_numeric(action_view[col], errors="coerce").round(2)
        st.dataframe(action_view, use_container_width=True, hide_index=True)
        buys=action_view[action_view["Buy Call"].fillna("").astype(str).str.startswith("BUY")].copy()
        if not buys.empty:
            st.markdown("### 🟢 Actionable BUY candidates")
            buy_cols=["Symbol","Timeframe","Buy Call","Confidence","Current","Stop Loss","Target 1","Target 2","Support","Resistance","Volume Ratio","R:R T1"]
            st.dataframe(buys[buy_cols], use_container_width=True, hide_index=True)
        else:
            st.info("No candidate currently has enough support/volume/reward confirmation for a BUY call.")
